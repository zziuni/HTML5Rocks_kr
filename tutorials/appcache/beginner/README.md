...
개발자는 Application Cache(AppCache라고도 함.)를 사용하여 캐싱해야할 파일들을 지정할 수 있고 이를 통해서 이 파일들에 오프라인 사용자가 접근할 수 있게 만든다.  그러면 사용자가 오프라인에서 새로고침을 눌러도, 여러분의 앱은 정상적으로 실행되고 작동할 것이다. 

캐시 메니페스트 파일(cache manifest file)
----------------------------------
...
웹 앱에서 application cache를 가능하게 하기위해서는 다음처럼 문서의 html 태그에 *manifest* 속성을 넣어야 한다.
...
캐싱하고자 하는 모든 웹 어플리케이션의 페이지는 *manifest* 속성을 가지고있어야 한다. *manifest* 속성을 지정하지 않으면 페이지는 캐싱되지 않는다. (메니페스트 파일안에 그 페이지 파일명이 명확하게 들어있지 않는 한 말이다.) 이 말은 반대로 사용자가 *manifest* 속성이 지정된 페이지에 접근하면 그 페이지는 메니페스트 파일 안에 명시적으로 포함되어있지 않아도 암묵적으로 application cache에 추가된다 말이기도 하다. 따라서 캐싱하려는 페이지들을 메니페스트 파일에 포함시킬 필요는 없다. 
...
*manifest* 속성에는 상대주소와 절대 주소 모두 쓸 수 있다. 하지만 절대 주소는 동일 원본 정책(same origin policy)에서만 가능하다. 메니페스트 파일의 확장자는 뭐든 가능하지만 서버에서 mime-type 지정이 필요하다. 다음을 보자 .
...
메니페스트 파일은 반드시 mime-type을 *text/cache-manifest*으로 제공되야 한다. 그러므로 웹서버나 *.htaccess* 에서 커스텀 파일 타입을 추가해야 할 지도 모른다. 
...
다음은 Google App Engine의 app.yaml 파일 설정의 예다. 
...
웹 페이지에 이 메니페스트 파일을 지정하면 4개의 파일을 캐싱할 것이다. 
...
* 사이트 당 캐시 데이터의 한계는 5메가다. 그러나 [Chrome Web Store](http://code.google.com/chrome/apps/docs/developers_guide.html)용 앱을 짜는 거라면, *unlimitedStorage*를 사용하여 용량 제한을 풀 수 있다. 
* 메니페스트 파일 자체나 그 안에 정의된 리소스 다운로드에 문제가 발생하면, 캐시 업데이트 전체의 실패를 의미한다. 그래서 브래우저는 이전에 캐싱해둔 application cache를 사용하게 될 것이다. 
...
'#'로 시작하는 줄은 주석이다. 하지만 다른 목적으로도 쓰인다. App Cache는 메니페스트 파일이 변경될 때만 업데이트 된다. 예를 들어 여러분이 이미지를 수정하거나 자바스크립트 함수를 변경한다고, 캐시가 갱신 되진 않는다. 그러므로 **브라우저가 캐시된 파일을 갱신할 수 있도록 메니페스트 파일을 수정해야한다.** 바로 이때 주석이 사용되는데, 주석에 버전, 파일 해시 값, 타임스템프를 포함시켜서 생성하는 것이 사용자가 마지막 버전을 가져가도록 하는 확실한 방법이다. **캐시 업데이트** 절에서 설명할 캐시 갱신 방법을 이용하여, 여러분은 캐시를 프로그래밍 적으로해서 업데이트 할 수도 있다. 
...
*CACHE:*<br/>
전체 중 기본 섹션이다. 이 헤더 밑(아니면, CACHEMANIFEST 바로 아래)의 파일 목록은 처음 접속시 다운로드되서 확실하게 캐싱된다. 
...
FALLBack:<br/>
리소스에 접근할 없을때 필요한 폴백(fallback, 대안)를 정의하는 섹션이다. 필수 세션은 아니다. 첫 URI는 리소스고 두번째가 폴백이다. 두 URI는 반드시 동일 종류여야 하고 메니페스트 파일과 동일 원본이여야 한다. 와일드카드(*)가 사용될 수 있다. 
...
예제로 다음 메니페스트 파일을 보면 사용자가 오프라인에서 사이트의 루트를 접근하려 할때 출력될 페이지, 즉 캐시로만 실행할 페이지(offline.html)를 정의했다. 그리고 이를 제외한 모든 리소스는 인터넷 접속이 필요하다고 선언되어 있다. 
...
**주의**: 메니페스트 파일을 참조하는 HTML 파일은 자동으로 캐시된다. 하지만 메니페스트 파일에 포함시킬 필요없다. 적지 말기를 권장한다.  
...
**주의**: SSL로 제공된 페이지의 HTTP 캐시 해더와 부과된 캐싱 제한은 application cache의 케시 메니페스트 파일이 덥어 버린다. 따라서 https로 제공되는 페이지도 오프라인에서 작동되게 만들 수 있다. 
...
어플리케이션은 다음 중 하나가 일어날 때 까지는 현재 캐시된 것으로 오프라인을 유지한다. 
	...
	window.addEventListener('load', function(e) {
	  ...
	  window.applicationCache.addEventListener('updateready', function(e) {
	    if (window.applicationCache.status == window.applicationCache.UPDATEREADY) {
	      // 브라우저가 새 앱 캐시를 다운받는다. 
	      // 캐시를 교채하고, 따끈따끈한 새 파일을 받기위해 페이지 리로드.
